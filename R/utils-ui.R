...
se_info_ui <- function(se, organism = NULL, annotation = NULL) {
  ...
  tagList(
    tags$ul(
      style = "list-style: none; padding-left: 15px; margin: 5px 0;",
      info_rows
    )
  )
}
...
build_symbol_select_ui <- function(annot_cols, primary_sel) {
  exclude <- c(
    ...
    "score", "strand", "frame", "attributes", "Geneid"
  )
...
msigdb_controls_ui <- function(collection_input_id,
  ...
  organism_note <- if (!is.null(organism_label)) {
    ...
    tags$p(
      ...
      icon("globe"), strong("MSigDB for: "), organism_label, note
    )
  }
  ...
  tagList(
    ...
    tags$div(
      ...
      tags$a(
        ...
        target = "_blank",
        icon("external-link-alt"), " MSigDB browser"
      )
    )
  )
}
