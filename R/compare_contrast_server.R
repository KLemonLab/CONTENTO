  ...
  output$leadingEdgeTable <- renderDT(
    {
      ...
      # Get gene symbols from DE results (symbol column may not exist yet)
      gene_symbols <- state$de_df() %>%
        select(Geneid, any_of(c("symbol"))) %>%
        distinct()
      ...
      id_cols <- intersect(c("gene", "symbol"), colnames(df))
      df <- df %>%
        select(all_of(id_cols), everything()) %>%
        mutate(across(-all_of(id_cols), ~ ifelse(. == 1, "TRUE", "FALSE")))
