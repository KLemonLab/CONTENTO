...
- **metadata(se)**: a list with, at minimum, `contrasts` (a character vector of unique contrast names matching the `rowData` suffixes described above). Optional fields `organism` and `annotation` unlock MSigDB-based GSEA and annotation loading, respectively (see the annotation article for details). Two further optional fields fine-tune the CONTENTO experience:
  - `db_species`: character string, `"HS"` or `"MM"`. Overrides CONTENTO's automatic mapping from `organism` to an `msigdbr` database species. Use this if your organism isn't natively supported by `msigdbr` but you still want ortholog-mapped MSigDB gene sets (e.g. querying human Hallmark gene sets for a non-human, non-mouse organism).
...
| MSigDB GSEA | `metadata(se)$organism` matching a valid `msigdbr` species (or `metadata(se)$db_species` override) |
...
convert_to_SE <- function(dds, contrasts_df, expr_matrix,
  ...
  metadata(se) <- list(
    ...
    date = Sys.Date() # records the date of SE creation for reproducibility
  )
...
SE = convert_to_SE(
  ...
  # db_species is left NULL here because "Homo sapiens" already maps natively to "HS";
  # set e.g. db_species = "HS" to force human MSigDB gene sets for an organism
  # msigdbr doesn't natively support.
)
...
- [ ] `metadata(se)$db_species`, if set, is exactly `"HS"` or `"MM"` — overrides automatic msigdbr species routing based on `organism`.
