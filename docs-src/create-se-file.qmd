...
- **metadata(se)**: a list with, at minimum, `contrasts` (a character vector of unique contrast names matching the `rowData` suffixes described above). Optional fields `organism` and `annotation` unlock MSigDB-based GSEA and annotation loading, respectively (see the annotation article for details). Two further optional fields fine-tune the CONTENTO experience:
