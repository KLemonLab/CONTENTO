...
CONTENTO expects a single `SummarizedExperiment` (SE) object (`.rds`) containing your normalized expression data, differential expression results for every contrast, and descriptive metadata. This document walks through building that object from RSEM/tximport counts through DESeq2, using a helper function (`convert_to_SE()`) that assembles the final SE from a fitted `dds` and a long-format table of contrast results.

A CONTENTO-compatible SE has four required components:

- **assays**: `counts` (raw), `normalized` (size-factor normalized), and `vst` (variance-stabilized). The `vst` assay is required for the Expression Plot, Heatmap, and GESECA co-expression features.
...
- **metadata(se)**: a list with, at minimum, `contrasts` (a character vector of contrast names matching the rowData suffixes above). Optional fields `organism` and `annotation` unlock MSigDB-based GSEA and annotation loading, respectively (see the annotation article for details).

| Feature | Required SE component |
|---|---|
| DE tables & volcano plots | rowData contrast columns |
| Expression Plot / Heatmap / GESECA | `vst` assay |
...
3. **Exploratory DESeq2 Analysis** — fit a simple model and inspect a PCA to sanity-check the data before committing to a final design.
4. **Differential Expression Analysis** — fit the full design and extract one or more contrasts.
5. **Create SummarizedExperiment (SE) Object** — assemble assays, rowData, colData, and metadata with `convert_to_SE()`.

**Example 1** covers a single two-group comparison (one contrast). **Example 2** covers a multi-factor design with an interaction term, producing several related contrasts — use it as the template if your experiment has more than one categorical variable of interest (e.g. treatment × condition).
...
The `convert_to_SE()` helper below builds the final SE object: it reshapes the long-format contrast results into wide rowData columns, pulls `counts`/`normalized`/`vst` assays from `dds`, attaches sample metadata as colData, optionally merges in variance-partition results, and records `contrasts`, `organism`, and `annotation` in `metadata(se)`. Reuse this same function for any design.
...
More complex experiment with RSV infecting HNO-ALI N=3 experiments  (HNO control vs RSV infected). TRUE/FALSE for virus with 4 different bacteria conditions (NB, Spn, Hin, Dpi). This design allows us to explore the interaction between virus and bacteria species, as well as the main effects of each factor. Because this design defines several distinct comparisons, `metadata(se)$contrasts` will hold multiple contrast names, each with its own set of rowData columns.
...
- [ ] `assays(se)` includes `vst` (required for expression/heatmap/GESECA features).
- [ ] `rowData(se)` has `<stat>_<contrast>` columns for every entry in `metadata(se)$contrasts`, with matching suffixes.
